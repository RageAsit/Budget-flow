...
export default function App() {
  ...
  const modeBalances = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      if (t.type === 'income') {
        ...
        else if (t.mode === 'Online') online += amt;
        else if (t.mode === 'FD(Saving)') fd += amt;
        total += amt;
      } else {
        if (t.mode === 'Cash') cash -= amt;
        else if (t.mode === 'Online') online -= amt;
  ...
  const generateInsight = async () => {
    if (filteredTransactions.length === 0) {
      setAiInsight("No transactions found for this month. Add some spending to get insights! 📉");
      return;
    }
    ...
    try {
      ...
      const prompt = `
        Act as a savvy, modern financial coach. Analyze this monthly budget data for ${selectedMonth}:
        - Income: ₹${monthlySummary.income}
        - Expense: ₹${monthlySummary.expense}
        - Net Balance: ₹${monthlySummary.balance}
        - Top Spending Categories: ${topCategories}
        
        Provide 3 short, punchy insights. Use emojis. Keep it dark-mode friendly (no bright highlights).
      `;
      ...
      console.error(error);
      setAiInsight("AI is currently taking a coffee break. Try again later! ☕");
    }
  ...
  const handleSmartFill = async () => {
    ...
    try {
      const prompt = `
        ...
        - category: string (Pick one closest match from: Food, Transport, Shopping, Utilities, Entertainment, Health, Others, Salary, Freelance, Investment, Gift)
        - type: string ('expense' or 'income')
        - mode: string ('Cash', 'Online', 'FD(Saving)') - Default to 'Online' if not specified.
      `;
      ...
      console.error(error);
      alert("Couldn't understand that text. Please try again.");
    }
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-200 pb-24 md:pb-0 selection:bg-cyan-500/30">
      ...
      {/* Header */}
      <header className="bg-slate-900/50 backdrop-blur-xl border-b border-slate-800 pt-8 pb-24 px-6 relative z-0">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1 font-medium">Financial Command Center</p>
          </div>
          ...
          <div className="bg-slate-800 p-1 rounded-xl flex items-center border border-slate-700 shadow-inner">
            ...
            <input 
              ...
              className="bg-transparent text-white text-sm font-medium px-3 py-1.5 outline-none border-none cursor-pointer"
            />
          </div>
        </div>
      </header>
          ...
          <div className="space-y-8">
            ...
            <section>
              <div className="bg-gradient-to-br from-violet-900/50 to-fuchsia-900/50 rounded-2xl p-6 border border-white/10 shadow-xl backdrop-blur-sm relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-fuchsia-500/10 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"></div>
                ...
                <div className="flex justify-between items-start mb-4 relative z-10">
                  ...
                  {!aiInsight && (
                    <button 
                      ...
                      className="bg-white/10 hover:bg-white/20 text-white text-xs font-bold px-4 py-2 rounded-full transition-all disabled:opacity-50 border border-white/10"
                    >
                      {isGeneratingInsight ? 'Analyzing...' : 'Analyze Spending'}
                    </button>
                  )}
                </div>
                
                {aiInsight ? (
                  <div className="bg-black/20 rounded-xl p-5 text-slate-200 text-sm leading-relaxed whitespace-pre-line animate-in fade-in duration-500 border border-white/5 relative z-10">
                    {aiInsight}
                    <button 
                  ...
                  <p className="text-slate-300 text-sm relative z-10">
                    Get smart insights about your {new Date(selectedMonth + '-01').toLocaleString('default', { month: 'long' })} finances instantly.
                  </p>
                )}
              </div>
            </section>
            ...
            {/* 3. Monthly Overview */}
            <section>
              <h2 className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-4 ml-1">
                {new Date(selectedMonth + '-01').toLocaleString('default', { month: 'long', year: 'numeric' })} Overview
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                
                {/* Income vs Expense Card */}
                <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-lg flex flex-col justify-center relative overflow-hidden">
                   <div className="flex items-center justify-between mb-8 relative z-10">
                      <div>
                        <p className="text-sm text-slate-400 font-medium">Income</p>
                        <p className="text-3xl font-bold text-emerald-400 mt-1">+₹{monthlySummary.income.toLocaleString('en-IN')}</p>
                      </div>
                   ...
                   <div className="flex items-center justify-between relative z-10">
                      <div>
                        <p className="text-sm text-slate-400 font-medium">Expense</p>
                        <p className="text-3xl font-bold text-rose-400 mt-1">-₹{monthlySummary.expense.toLocaleString('en-IN')}</p>
                      </div>
                ...
                <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-lg">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <h2 className="text-lg font-bold text-white">Spending Split</h2>
                  </div>
                ...
                <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-lg">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
                              <span className="bg-slate-800 px-2 py-0.5 rounded border border-slate-700 text-slate-400">{t.mode}</span>
                              <span>•</span>
                  ...
                  {filteredTransactions.length === 0 && (
                    <div className="py-16 text-center">
                      ...
                      <p className="text-slate-500">No transactions found for {selectedMonth}.</p>
                    </div>
                  )}
                </div>
            </section>
          ...
          <div className="max-w-lg mx-auto mt-8 space-y-6">
              ...
              <div className="bg-slate-950 rounded-xl p-6">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e.g. Spent 500 on KFC via GPay"
                    className="flex-1 px-4 py-3 rounded-xl bg-slate-900 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none text-sm text-white placeholder:text-slate-600 transition-all"
                  />
                  <button
                    onClick={handleSmartFill}
                    disabled={isSmartFilling || !smartInput}
                    className="bg-cyan-500 hover:bg-cyan-600 text-white px-5 py-2 rounded-xl font-medium text-sm transition-colors disabled:opacity-50 flex items-center gap-2 shadow-lg shadow-cyan-500/20"
                  >
                    {isSmartFilling ? <Sparkles className="animate-spin" size={18} /> : <Sparkles size={18} />}
            ...
            <div className="bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-800 shadow-xl">
              <h2 className="text-xl font-bold mb-6 flex items-center gap-3 text-white">
                <div className="bg-cyan-500/10 p-2 rounded-full border border-cyan-500/20 text-cyan-400">
                  <Plus size={20} />
                </div>
                Manual Entry
              </h2>
              <form onSubmit={handleAddTransaction} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 bg-slate-950 p-1 rounded-xl border border-slate-800">
                  <button
                    type="button"
                    onClick={() => { setType('expense'); setMode('Online'); }}
                  ...
                  <button
                    type="button"
                    onClick={() => { setType('income'); setMode('Online'); }}
                    className={`p-3 rounded-lg text-center font-medium transition-all ${
                      type === 'income' 
                ...
                <div>
                  ...
                  <div className="relative">
                    <IndianRupee className="absolute left-4 top-3.5 text-slate-500" size={18} />
                    <input
                      type="number"
                      step="0.01"
                      required
                      ...
                      className="w-full pl-10 pr-4 py-3.5 rounded-xl bg-slate-950 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none text-white text-lg transition-all placeholder:text-slate-700"
                      placeholder="0.00"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3.5 rounded-xl bg-slate-950 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none text-white transition-all placeholder:text-slate-700"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-4 py-3.5 rounded-xl bg-slate-950 border border-slate-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none text-white appearance-none cursor-pointer"
                      >
                        {type === 'expense' ? (
                          <>
                          ...
                          <>
                            <option value="Online">Online</option>
                            <option value="Cash">Cash</option>
                            <option value="FD(Saving)">FD (Saving)</option>
                          </>
                        )}
                      </select>
                ...
                <div className="flex gap-4 mt-8 pt-4 border-t border-slate-800">
                  <button
                    ...
                    className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium py-3.5 rounded-xl transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-[2] bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-3.5 rounded-xl transition-colors shadow-lg shadow-cyan-500/20 flex justify-center items-center gap-2"
                  >
           ...
           <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden mt-8 shadow-xl">
             <div className="p-6 border-b border-slate-800 flex justify-between items-center">
               <h2 className="text-xl font-bold text-white">
                 {new Date(selectedMonth + '-01').toLocaleString('default', { month: 'long', year: 'numeric' })} History
               </h2>
               <button onClick={() => setActiveTab('dashboard')} className="text-sm text-cyan-400 hover:text-cyan-300">Back</button>
             </div>
             <div className="divide-y divide-slate-800">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
                          ...
                          <span>{t.category}</span>
                          <span>•</span>
                          <span>{t.createdAt.toLocaleDateString()} {t.createdAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className={`font-bold text-lg ${t.type === 'income' ? 'text-emerald-400' : 'text-slate-200'}`}>
                      {t.type === 'income' ? '+' : '-'}₹{Number(t.amount).toLocaleString('en-IN')}
                    </span>
                    <button 
              ...
              {filteredTransactions.length === 0 && (
                <div className="py-20 text-center text-slate-500">No transactions found.</div>
              )}
             </div>
           </div>
        )}
      ...
      <button
        ...
        className="md:hidden fixed bottom-6 right-6 bg-cyan-500 text-white p-4 rounded-full shadow-lg shadow-cyan-500/40 hover:bg-cyan-600 transition-transform hover:scale-105 active:scale-95 z-50"
      >
        <Plus size={28} />
      </button>
      ...
      <div className="hidden md:block fixed bottom-8 right-8 z-50">
         {activeTab !== 'add' && (
            <button
              onClick={() => setActiveTab('add')}
              className="bg-cyan-500 text-white px-6 py-3 rounded-full shadow-lg shadow-cyan-500/30 hover:bg-cyan-600 font-bold flex items-center gap-2 transition-transform hover:scale-105"
            >
